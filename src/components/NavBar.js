...
const NavBar = () => {
	const dropdown = [
		...
		{
			url: 'products/Esctacy',
			dropItem: 'Ecstacy'
		},
		{
			url: 'products/Euphorya',
			dropItem: 'Euphorya'
		},
		{
			url: 'products/Other',
					...
					<Nav className="nav">
						<NavDropdown title="Brend">
							{dropdown.map((item, index) => (
								<NavDropdown.Item onClick={() => Navigate(item.url)}>{item.dropItem}</NavDropdown.Item>
							))}

							{/* <NavDropdown.Divider /> */}
						</NavDropdown>
						<button onClick={() => Navigate('New')} className="na">
							In New
						</button>
						<button onClick={() => Navigate('About')} className="na">
							About-Us
						</button>
						<button onClick={() => Navigate('Contact')} className="na">
							contact-us
						</button>
					</Nav>
					<Nav className="d-flex ms-auto mr-2">
						<button onClick={() => Navigate('/')} className="na">
							<img src={logoOne} />
						</button>
						<button onClick={() => Navigate('/')} className="na">
							<img src={call} />
